...
public class MemoDAO {
	...
	public void insertMemo(String name, String passwd, String subject, String content, String email) {
		...
		try {
			...
			sql = "INSERT INTO memo (num,name,passwd,subject,content,email,reg_date) "
					+ "VALUES (memo_seq.nextval,?,?,?,?,?,SYSDATE)"; //nextval - 다음 시퀀스를 읽어옴
	...
	public void selectMemo() {
		...
		try {
			...
			//JDBC 4단계
			rs = pstmt.executeQuery(); //resultset에 정보를 담는다
			
			if(rs.next()) { //더이상 저장된 데이터가 없을 때 출력되지 않게 만들기
	...
	public void selectDetailMemo(int num) {
		...
		try {
			...
			//JDBC 4단계 : SQL문을 실행해서 한 건의 레코드를 ResultSet에 담음
			...
			if(rs.next()) {
				...
				if(email==null) email = ""; //이메일이 null일 경우 비어있음
	...
	//글 수정, num은 primary key라 변경x, name,passwd,subject,content,email만 변경하기
